...
import {updateInquiryItems, type InquiryItemResponse, type InquiryItemRequest} from '../../services/api';
...
const fromInquiryItem = (item: InquiryItemResponse): PartItem => ({
  ...
  images: [
    item.image1Url ? {uri: item.image1Url, name: 'image1.jpg'} : null,
    item.image2Url ? {uri: item.image2Url, name: 'image2.jpg'} : null,
    item.image3Url ? {uri: item.image3Url, name: 'image3.jpg'} : null,
  ],
  isBrandDropdownOpen: false,
});
...
const ChevronIcon = ({color = '#e5383b', rotated = false}: {color?: string; rotated?: boolean}) => (
  <Svg width={20} height={20} viewBox="0 0 24 24" fill="none" style={rotated ? {transform: [{rotate: '180deg'}]} : undefined}>
...
const PlusIcon = () => (
  <Svg width={24} height={24} viewBox="0 0 32 32" fill="none">
    <Path d="M16 6V26M6 16H26" stroke="#e5383b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
  </Svg>
);
...
export default function EditInquiryOverlay({
  ...
  // Initialise parts when overlay opens
  useEffect(() => {
    if (isOpen) {
      setParts(
        initialItems.length > 0
          ? initialItems.map(fromInquiryItem)
          : [createEmptyPart()],
      );
    }
  }, [isOpen, initialItems]);
    ...
    try {
      if (Platform.OS === 'android') {
        const granted = await PermissionsAndroid.request(
          ...
          {title: 'Audio Permission', message: 'Needed to record audio.', buttonNeutral: 'Ask Me Later', buttonNegative: 'Cancel', buttonPositive: 'OK'},
        );
  ...
  const stopRecording = async () => {
    try {
      ...
      if (partId) updatePart(partId, {audioPath: path, audioDuration: duration});
      setIsRecording(false);
      setRecordingPartId(null);
      recordingPartIdRef.current = null;
    } catch (e) {
    ...
    try {
      if (playingPartId === partId) {
        ...
        audioRecorderPlayer?.addPlayBackListener(e => {
          if (e.currentPosition >= e.duration) setPlayingPartId(null);
        });
      }
    } catch (e) {
    ...
    launchImageLibrary({mediaType: 'photo', quality: 0.8}, res => {
      if (!res.didCancel && !res.errorCode && res.assets?.[0]) {
        ...
        setParts(prev => {
          ...
          newImages[firstEmpty] = {uri: a.uri!, name: a.fileName || `photo_${Date.now()}.jpg`};
  ...
  const handleSave = async () => {
    ...
    setParts(prev =>
      prev.map(p => {
        const hasErr = !p.partName.trim() || !p.preferredBrand || !p.quantity.trim() || (!p.remark.trim() && !p.audioPath);
    ...
    try {
      setSubmitting(true);
      const items: InquiryItemRequest[] = parts.map(p => ({
        partName: p.partName,
        preferredBrand: p.preferredBrand,
        quantity: parseInt(p.quantity, 10) || 1,
        remark: p.remark,
        audioUrl: p.audioPath ?? undefined,
        audioDuration: p.audioDuration || undefined,
        image1Url: p.images[0]?.uri ?? undefined,
        image2Url: p.images[1]?.uri ?? undefined,
        image3Url: p.images[2]?.uri ?? undefined,
      }));

      const result = await updateInquiryItems(inquiryId, items);
      if (result.success) {
        setAppAlert({
          type: 'success',
        ...
        {part.isExpanded && (
          <View style={styles.accordionContent}>
            ...
            <View style={styles.fieldWrap}>
              <View style={[styles.inputBorder, !!part.partName && styles.inputFilled, part.hasAttemptedSubmit && !part.partName.trim() && styles.inputError]}>
              ...
              {part.hasAttemptedSubmit && !part.partName.trim() && <Text style={styles.errorMsg}>Please add part name</Text>}
            </View>
            ...
            <View style={[styles.fieldWrap, part.isBrandDropdownOpen && {zIndex: 10}]}>
              <TouchableOpacity
                ...
                style={[styles.inputBorder, styles.row, !!part.preferredBrand && styles.inputFilled, part.hasAttemptedSubmit && !part.preferredBrand && styles.inputError]}>
                {!!part.preferredBrand && <Text style={styles.floatLabel}>Preferred Brand</Text>}
                <Text style={[styles.inputText, {flex: 1, padding: 9}, !part.preferredBrand && {color: '#828282'}]}>
                  {part.preferredBrand || 'Preferred Brand'}
                </Text>
                <ChevronIcon color="#e5383b" rotated={part.isBrandDropdownOpen} />
              </TouchableOpacity>
              {part.isBrandDropdownOpen && (
                <View style={styles.dropdownList}>
                  {brandOptions.map(opt => (
                    <TouchableOpacity key={opt.id} onPress={() => updatePart(part.id, {preferredBrand: opt.name, isBrandDropdownOpen: false})} style={styles.dropdownItem}>
                      <Text style={styles.dropdownItemText}>{opt.name}</Text>
                    </TouchableOpacity>
                  ))}
                </View>
              )}
              {part.hasAttemptedSubmit && !part.preferredBrand && <Text style={styles.errorMsg}>Please select preferred brand</Text>}
            </View>

            {/* Quantity */}
            <View style={styles.fieldWrap}>
              <View style={[styles.inputBorder, !!part.quantity && styles.inputFilled, part.hasAttemptedSubmit && !part.quantity.trim() && styles.inputError]}>
              ...
              {part.hasAttemptedSubmit && !part.quantity.trim() && <Text style={styles.errorMsg}>Please add quantity</Text>}
            </View>

            {/* Remark + Record */}
            <View style={styles.fieldWrap}>
              {!isThisRecording && !part.audioPath && (
                <View style={[styles.inputBorder, styles.row, styles.remarkPad, !!part.remark && styles.inputFilled, part.hasAttemptedSubmit && !part.remark.trim() && !part.audioPath && styles.inputError]}>
                  {!!part.remark && <Text style={styles.floatLabel}>Remark</Text>}
                  <TextInput value={part.remark} onChangeText={v => updatePart(part.id, {remark: v})} placeholder="Remark" placeholderTextColor="#828282" style={[styles.inputText, {flex: 1}]} />
                  <TouchableOpacity onPress={() => startRecording(part.id)} disabled={isRecording && recordingPartId !== part.id} style={styles.recordBtn}>
                    <MicIcon />
              ...
              {isThisRecording && (
                <View style={styles.recordingRow}>
                  ...
                  <TouchableOpacity onPress={stopRecording} style={styles.stopBtn}><StopIcon /></TouchableOpacity>
                </View>
              )}
              {!!part.audioPath && !isThisRecording && (
                <View style={[styles.inputBorder, !!part.remark && styles.inputFilled]}>
                  {!!part.remark && <Text style={styles.floatLabel}>Remark</Text>}
                  <TextInput value={part.remark} onChangeText={v => updatePart(part.id, {remark: v})} placeholder="Remark" placeholderTextColor="#828282" style={styles.inputText} />
                </View>
              )}
              {part.hasAttemptedSubmit && !part.remark.trim() && !part.audioPath && <Text style={styles.errorMsg}>Please add remark</Text>}
            </View>
            ...
            <View style={styles.mediaGrid}>
              {!!part.audioPath && (
                <View style={styles.audioCard}>
                  <TouchableOpacity onPress={() => togglePlayback(part.id, part.audioPath!)} style={styles.audioPlayBtn}>
                    {isThisPlaying ? <PauseIcon /> : <PlayIcon />}
                  </TouchableOpacity>
                  <Text style={styles.audioDuration}>{formatTime(part.audioDuration)}</Text>
                  <TouchableOpacity onPress={() => updatePart(part.id, {audioPath: null, audioDuration: 0})} style={styles.mediaDeleteBtn}>
              ...
              {part.images.map((img, imgIdx) =>
                img ? (
                  <View key={imgIdx} style={styles.imageCard}>
                    <Image source={{uri: img.uri}} style={styles.imageFull} resizeMode="cover" />
      ...
      <View style={styles.sheet}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.countBadge}>
            <Text style={styles.countBadgeText}>{parts.length} {parts.length === 1 ? 'Part' : 'Parts'}</Text>
          </View>
        </View>
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={styles.formPad}>
            ...
            <TouchableOpacity onPress={addPart} style={styles.addAnotherBtn}>
            ...
            <TouchableOpacity onPress={handleSave} disabled={submitting} style={[styles.saveBtn, submitting && styles.saveBtnDisabled]}>
              <Text style={styles.saveBtnText}>SAVE CHANGES ({parts.length} {parts.length === 1 ? 'PART' : 'PARTS'})</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </View>
...
const styles = StyleSheet.create({
  ...
  handle: {width: 40, height: 4, backgroundColor: '#dadada', borderRadius: 2, alignSelf: 'center', marginTop: 12, marginBottom: 8},

  header: {flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: 20, paddingBottom: 16},
  backBtn: {padding: 4},
  headerTitle: {flex: 1, fontSize: 22, fontWeight: '700', color: '#000'},
  countBadge: {backgroundColor: '#f0f0f0', borderRadius: 20, paddingHorizontal: 12, paddingVertical: 4},
  countBadgeText: {fontSize: 14, color: '#828282'},

  formPad: {paddingHorizontal: 20, paddingBottom: 32, gap: 12},

  accordionWrap: {},
  accordionHeader: {flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#e5383b', borderRadius: 8, paddingHorizontal: 16, paddingVertical: 10},
  ...
  qtyBadge: {backgroundColor: 'rgba(255,255,255,0.2)', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 2},
  ...
  inputBorder: {borderWidth: 1, borderColor: '#d3d3d3', borderRadius: 8, paddingHorizontal: 16, paddingVertical: 4, position: 'relative'},
  inputFilled: {borderColor: '#e5383b'},
  inputError: {borderColor: '#e5383b', backgroundColor: '#ffe0e0'},
  floatLabel: {position: 'absolute', top: -8, left: 12, backgroundColor: '#fff', paddingHorizontal: 4, fontSize: 11, color: '#828282', zIndex: 1},
  inputText: {fontSize: 15, color: '#000'},
  errorMsg: {fontSize: 12, color: '#e5383b', marginTop: 4},

  dropdownList: {borderWidth: 1, borderColor: '#d3d3d3', borderRadius: 8, marginTop: 4, backgroundColor: '#fff', elevation: 4, shadowColor: '#000', shadowOffset: {width: 0, height: 2}, shadowOpacity: 0.1, shadowRadius: 4},
  ...
  addAnotherBtn: {height: 52, borderWidth: 1, borderColor: '#e5383b', borderStyle: 'dashed', borderRadius: 8, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8},
  ...
  saveBtnText: {fontSize: 16, fontWeight: '600', color: '#fff', letterSpacing: 1},
});
